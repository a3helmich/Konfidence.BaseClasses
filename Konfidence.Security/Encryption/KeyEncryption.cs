    ...
    private void GetKeyContainer(string containerName)
    {
        ...
        try
        {
            ...
            if (!RsaProvider.IsAssigned())
            {
                ...
                {
                    // No cleanup needed: this catch only runs from inside the "not assigned yet"
                    // branch, and a failed constructor call never assigns RsaProvider either.
            ...
            if (_maxBytesServer > 0 && RsaProvider.KeySize != _maxBytesServer * 8)
            {
                try
                {
                    // Delete() swallows CryptographicException internally and reports failure
                    // through its return value, so it needs no handler of its own here.
                    Delete();
