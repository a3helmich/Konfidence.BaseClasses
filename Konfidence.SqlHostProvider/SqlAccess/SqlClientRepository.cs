...
internal class SqlClientRepository : IDataRepository
{
    ...
    private readonly IDefaultDatabaseProvider _defaultDatabaseProvider;

    public SqlClientRepository(IClientConfig clientConfig) : this(clientConfig, new AppConfigDefaultDatabaseProvider())
    {
    }

    internal SqlClientRepository(IClientConfig clientConfig, IDefaultDatabaseProvider defaultDatabaseProvider)
    {
        _clientConfig = clientConfig;
        _defaultDatabaseProvider = defaultDatabaseProvider;
    }
    ...
    private SqlDatabase GetDefaultDatabase()
    {
        if (_defaultDatabaseProvider.TryGetDefaultConnectionString(out string connectionString))
        {
            return SqlDatabaseFactory.Create(connectionString);
        }
