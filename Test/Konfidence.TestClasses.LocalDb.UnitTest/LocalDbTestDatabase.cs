...
/// code paths via the generated Dl.* classes in Konfidence.Integration.TestClasses without needing
/// a live, network-reachable SQL Server - the way the existing IntegrationTest projects require.
/// This makes these tests runnable on GitHub Actions (which ships SQL Server LocalDB on
/// windows-latest, but has no route to the internal dev server).
///
...
public sealed class LocalDbTestDatabase
{
    ...
    {
        _workingDirectory = Path.Combine(Path.GetTempPath(), "Konfidence.TestClasses.LocalDb.UnitTest", Guid.NewGuid().ToString("N"));
