...
/// code paths (BaseDataItem CRUD via the generated Dl.* classes in Konfidence.Integration.TestClasses)
/// without needing a live, network-reachable SQL Server - the way the existing IntegrationTest
/// projects require. This makes these tests runnable on GitHub Actions (which ships SQL Server
/// LocalDB on windows-latest, but has no route to the internal dev server).
///
...
///
/// The database name is per-process (not a fixed "TestClassGenerator") because two processes
/// sharing one attached database can race at the SQL level. net9.0 and net10.0 test hosts can run
/// as genuinely concurrent processes under some test runners, so per-process isolation is the only
/// fully race-free option.
/// </summary>
[TestClass]
public sealed class LocalDbTestDatabase
{
    ...
    {
        _workingDirectory = Path.Combine(Path.GetTempPath(), "Konfidence.BaseDatabaseClasses.LocalDb.UnitTest", Guid.NewGuid().ToString("N"));
    ...
    // Dl.* classes resolve their connection through DependencyInjectionFactory.ConfigureDependencyInjection(),
    // which reads SqlClientSettings.json fresh on every call, so overwriting it here (before any test
    // runs) is enough to point every subsequent test at this process's own isolated database.
